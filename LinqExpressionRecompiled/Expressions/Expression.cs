namespace System.Linq.Expressions
{
    ...
    public abstract class Expression
    {
        ...
        private static bool TryQuote(System.Type parameterType, ref Expression argument)
        {
            System.Type type = typeof(LambdaExpression);
            if (TypeUtils.IsSameOrSubclass(type, parameterType) && parameterType.IsAssignableFrom(argument.GetType()))
            {
        ...
        private static void ValidateAccessorArgumentTypes(MethodInfo method, ParameterInfo[] indexes, ref ReadOnlyCollection<Expression> arguments)
        {
            if (indexes.Length > 0)
            {
                ...
                while (index < length)
                {
                    ...
                    if (!TypeUtils.AreReferenceAssignable(parameterType, expression.Type) && !TryQuote(parameterType, ref expression))
                    {
        ...
        private static void ValidateLambdaArgs(System.Type delegateType, ref Expression body, ReadOnlyCollection<ParameterExpression> parameters)
        {
            ...
            if (((method.ReturnType != typeof(void)) && !TypeUtils.AreReferenceAssignable(method.ReturnType, body.Type)) && !TryQuote(method.ReturnType, ref body))
            {
