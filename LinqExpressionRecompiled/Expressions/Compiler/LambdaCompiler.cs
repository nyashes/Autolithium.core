    ...
    internal sealed class LambdaCompiler
    {
        ...
        private void Emit(BlockExpression node, CompilationFlags flags)
        {
            ...
            for (int i = 0; i < (expressionCount - 1); i++)
            {
                ...
                if (flags3 != CompilationFlags.EmitAsNoTail)
                {
                    ...
                    {
                        emitAsTail = CompilationFlags.EmitAsTail;
                    }
                    else
                    {
                        emitAsTail = CompilationFlags.EmitAsMiddle;
                    }
                }
                else
                {
                    emitAsTail = CompilationFlags.EmitAsNoTail;
                }
                flags = UpdateEmitAsTailCallFlag(flags, emitAsTail);
        ...
        private void EmitDelegateConstruction(LambdaCompiler inner)
        {
            ...
            if (method != null)
            {
                this._boundConstants.EmitConstant(this, method, typeof(MethodInfo));
                this._ilg.EmitType(type);
                this.EmitClosureCreation(inner);
                this._ilg.Emit(OpCodes.Callvirt, typeof(MethodInfo).GetMethod("CreateDelegate", new Type[] { typeof(Type), typeof(object) }));
